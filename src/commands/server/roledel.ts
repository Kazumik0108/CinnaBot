...
export default class addrole extends Command {
    ...
    async run(message: CommandoMessage, { roleName }: promptArgs): Promise<null> {
        ...
        const reply = await message.reply('verify the role you want to delete.\nReact with 👍 to delete the role. React with 👎 to cancel the command.');
        ...
        msg.awaitReactions(filter, { max: 1, time: 10 * 1000, errors: ['time'] })
            .then(collected => {
                ...
                if (reaction.emoji.name === '👍') {
                    role.delete()
                        .then(() => message.reply(`the role \`${roleName}\` has successfully been deleted.`).then(confirmMsg => confirmMsg.delete({ timeout: 5000 })))
