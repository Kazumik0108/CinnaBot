// roleupdate.ts
import { ClientUser, ColorResolvable, EmbedFieldData, Guild, MessageEmbed, PermissionResolvable, Role, RoleData } from 'discord.js';
...
function getRoleEmbed(role: Role, maxPos: number): MessageEmbed {
    ...
    const fieldData: EmbedFieldData[] = [
        { name: 'Admin', value: role.permissions.toArray().includes('ADMINISTRATOR').toString().toUpperCase(), inline: false },
...
export default class addrole extends Command {
    ...
    async run(message: CommandoMessage, { roleName }: promptArgs): Promise<null> {
        ...
        Promise.resolve((message.guild as Guild).roles.cache.filter(role => role.name === roleName).first())
            .then(async role => {
                if (role) {
                    ...
                    const botRole = message.guild?.members.cache.get((message.client.user as ClientUser).id)?.roles.cache.filter(botRoles => botRoles.permissions.has('MANAGE_ROLES')).first();

                    // show current role properties to user and give instructions
                    const embedMessage = getRoleEmbed(role, botRole?.position! - 1);
                    ...
                    collector.on('collect', collect => {
                        ...
                        if (args.toLowerCase().startsWith(options[4])) {
                            ...
                            if (inputPos < botRole?.position! - 1) embedEdit.fields[3].value = `~~${embedMessage.fields[3].value}~~ ${inputPos}`;
                        }
                        ...
                        if (args.toLowerCase().startsWith(options[5])) {
                            const inputPerms = args.slice(options[5].length).trim().toUpperCase().split(/,*\s+,*/g);
                            ...
                            inputPerms.forEach(permission => {
                                ...
                                if (roleperms.includes(perm)) {
                                    ...
                                    // else, do nothing
                                    if (rolePerms.includes(perm)) {
                                        currPerms = currPerms.map(currPerm => (currPerm.includes(perm)) ? currPerm.replace(/~+/g, '') : currPerm);
                                    }
                        ...
                        if (args.toLowerCase().startsWith(options[6])) {
                            const inputPerms = args.slice(options[6].length).trim().toUpperCase().split(/,*\s+,*/g);
                    ...
                    collector.on('end', collected => {
                        ...
                        if (collected.every(msg => !msg.content.includes('cancel'))) {
                            ...
                            role.edit(roleData)
                                .then(() => message.reply(`the role \`${role.name}\` has successfully been added.`).then(confirmMsg => confirmMsg.delete({ timeout: 5000 })));
                        }
                        else {
        ...
        function getRoleData(edit: MessageEmbed, msg: MessageEmbed): RoleData {
            const name = () => {
                if (edit.description !== msg.description) {
                    return (edit.description as string).replace(msg.description as string, '').replace(/~+/g, '').trim();
                }
                else {
                    return undefined;
                }
            ...
            const perms = () => {
                if (edit.fields[4].value !== msg.fields[4].value) {
                    return edit.fields[4].value.split('\n').filter(permission => !permission.includes('~')).map(permission => permission.replace(/\++|\*+/g, '').trim());
                }
                else {
                    return undefined;
                }
